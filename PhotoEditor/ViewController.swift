...
class ViewController: UIViewController, UIScrollViewDelegate {
    ...
    @IBAction func rotateClockwise(_ sender: UIButton) {
        rotationAngle += 0.5
        //transformImage()
        let frame = CGRect(x: 0, y: 0, width: imageView.frame.height, height: imageView.frame.width )
        imageView.frame = frame
        imageView.image = imageRotatedByDegrees(oldImage: imageView.image!, deg: 90)
    }
    ...
    func transformImage(){
        ...
        containerViewWidthConstraint.constant = intrinsicSize.height
        containerViewHeightConstraint.constant = intrinsicSize.width
